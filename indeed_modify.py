...
def main():
    ...
    while True:
        # take next 10
        df_slice = df_orig.iloc[offset:offset + batch_size].copy()
        if df_slice.empty:
            break
        ...
        for _, orig in df_slice.iterrows():
            ...
            # Combine original columns + computed names + scraped fields
            name = (orig.get("name") or "").strip()
            parts = name.split()
            row = {
        ...
        # Build output DataFrame in the exact column order required
